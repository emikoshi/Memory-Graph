/**
 * $Id: mxConnectionHandler.java,v 1.31 2012-02-01 15:50:48 gaudenz Exp $
...
public class mxConnectionHandler extends mxMouseAdapter
{
	 ...
	 * 
	 */
	protected transient MouseEvent initial;
	...
	public void mouseDragged(MouseEvent e)
	{
		...
		{
			if (initial != null && first == null)
			{
				double dx = Math.abs(initial.getX() - e.getX());
				double dy = Math.abs(initial.getY() - e.getY());
				int tol = graphComponent.getTolerance();
				
				if (dx > tol || dy > tol)
				{
					start(initial, marker.getValidState());
					initial = null;
				}
			}
			
			if (e.getButton() == 0 || connectPreview.isActive())
			{
